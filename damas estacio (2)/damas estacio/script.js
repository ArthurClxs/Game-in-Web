window.onload = function () {
  ...
  */
  ...
  // Adicione este JS ao seu arquivo script.js
  ...
  updateDateTime(); // Chama a função imediatamente para exibir a data e hora ao carregar a página
  ...
  function updateScore() {
    ...
    document.getElementById('beigeScore').innerText = beigeScore;
  }

  function switchTurn() {
    isRedTurn = !isRedTurn;
    document.getElementById('redPlayer').classList.toggle('turn', isRedTurn);
    document.getElementById('beigePlayer').classList.toggle('turn', !isRedTurn);
  }

  // Exemplo de como incrementar o placar
  function incrementRedScore() {
    redScore++;
    updateScore();
  }

  function incrementBeigeScore() {
    beigeScore++;
    updateScore();
  }

  // Inicializa o placar e o turno
  updateScore();
  switchTurn();

  // Suponha que esta função seja chamada quando uma jogada é feita
  function onMove() {
    // Lógica para verificar se a jogada é válida e atualizar o estado do jogo

    // Se a jogada resulta em um ponto para o jogador atual
    if (isRedTurn) {
      incrementRedScore();
    } else {
      incrementBeigeScore();
